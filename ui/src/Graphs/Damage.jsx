...
export default function DamageGraph({ data }) {
  ...
  const countData = React.useMemo(() => {
    ...
    return Object.entries(freq).map(([damage, count]) => ({
  ...
  const damageData = React.useMemo(() => {
    if (countData.length === 0) return [];
  
    const values = countData.map(d => d.damage);
    const minValue = Math.min(...values);
    const maxValue = Math.max(...values);
  
    const range = [];
    for (let i = minValue; i <= maxValue; i++) {
      const existing = countData.find(d => d.damage === i);
      range.push({
        damage: i,
